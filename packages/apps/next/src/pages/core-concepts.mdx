...
### Primitive Components
 ...
 Primitive components are basic building blocks that can be used together to create more complex components.  Some examples of primitive components are `Box`, `Flex`, `Grid`, and `Text`.

### Complex Components

 Complex components are components that are made up of one or more building block components to represent a commonly used pattern or utility in our UI. Some examples of complex components are `TruncatedText` and `Card`.
...
Complex components are organized based on their category.  Some examples of this organization include `data display`, `inputs`, and `layout`.
 ...
 ## The `as` prop
 The `as` prop is a feature that all of our components get from [styled-components](https://www.styled-components.com). It allows you to pass a HTML tag or another component to a Component to be rendered as the base tag of that component along with all of it's styles and props.
