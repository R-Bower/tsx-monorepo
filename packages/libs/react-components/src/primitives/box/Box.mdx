...
| sx        | sx     |  `null` | See the [SX Prop](/sx-prop) page    |

## System props

Box components get the `COMMON` category of system props which includes `BORDER`, `COLOR`, `DISPLAY`, `SIZING`, and `SPACE`. Refer to the table below each property.

<StylePropsSearch propGroups={["color", "display", "sizing", "space", "border"]} />
