...
##### Solution
All of our primitives expose an `sx` prop that serves as a superset of CSS (contains all CSS properties/selectors in addition to custom ones).  Like with our primitives, these CSS values are theme-aware.
```
<Box sx={{
  ":hover": {
    bg: "bg.tertiary",
