...
With the SX prop, every available CSS property is valid.  This is powerful, but it's also easy to abuse.  For example, using the SX prop you can provide props to components that don't accept that prop by default.  Consider the `Box` primitive which doesn't include any `TYPOGRAPHY` props.
...
The SX prop is best used to apply styles that are otherwise impossible to apply using our existing system props.  Before using the SX prop, ensure that you can't apply the same styles using an existing component.
